...
const Category = () => {
  const saveCategory = useApi(API_URLS.createNewCategory); // will create new category
  ...
  const handleSave = async () => { // we save the data and go to next page
    await saveCategory.call(formData);
    setFormData( {
    name: '',
    description: '',
    status: 'active'
  ...
  return (
    <div className="w-screen border rounded-lg shadow-purple-600 ml-4 my-2 overflow-hidden p-4">
      <div className="flex items-center mb-4">
        <p className="text-lg font-semibold flex items-center gap-2"><a href="/category/view"><IoIosArrowRoundBack/></a>Add Category</p>
      </div>
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div>
          ...
          <input type="text" className="border rounded w-full p-2" name="description" value={formData.description} onChange={handleInputChange}/>
        </div>
        <div>
